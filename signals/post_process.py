...
def _calculate_fft_hr(ppg_signal, fs=28, low_pass=0.6, high_pass=3.3):
...
def _compute_macc_and_lag(pred_signal, gt_signal, fs=28, max_lag_seconds=2.0):
    """
    Computes MACC and best lag using standard (non-circular) cross-correlation.
    Restricts lag search to +/- max_lag_seconds to avoid nonsense large lags.
    """
    pred = np.squeeze(np.asarray(pred_signal))
    gt = np.squeeze(np.asarray(gt_signal))
    ...
    # Normalize
    corr /= np.sqrt(np.sum(pred**2) * np.sum(gt**2))

    lags = np.arange(-N + 1, N)
    max_lag_samples = int(round(max_lag_seconds * fs))

    # Restrict to a sensible lag window
    ...
    macc = float(corr[idx])
    best_lag_samples = int(lags[idx])
    best_lag_seconds = best_lag_samples / float(fs)

    #print(best_lag_seconds)

    return macc, best_lag_samples, best_lag_seconds


def _calculate_SNR(pred_ppg_signal, hr_label, fs=28, low_pass=0.6, high_pass=3.3):
...
def calculate_metric_per_video(predictions, labels, fs=28, diff_flag=True, use_bandpass=True, hr_method='FFT'):
    ...
    macc, _, time_lag = _compute_macc_and_lag(predictions, labels, fs=28)
    ...
    return hr_label, hr_pred, SNR, macc, predictions, time_lag