...
class NPSNR(nn.Module):
    def __init__(self,Lambda,LowF=0.6,upF=3.3,width=0.4):
...
class CosineSimilarityLoss(nn.Module):
    def __init__(self):
        ...
        self.cosine_similarity = nn.CosineSimilarity(dim=1)  # Use dimension 1 for batch dimension

    def forward(self, input1, input2):
        # Calculate cosine similarity
        cosine_sim = self.cosine_similarity(input1, input2)
        # Cosine similarity ranges from -1 to 1. To make a loss function, we often use 1 - cosine_sim
        loss = 1 - cosine_sim.mean()  # Average loss across the batch
        return loss